...
function UniteSettingsDOUBLY(){
	...
	var g_objProvider = new UniteProviderAdminDOUBLY();
	...
	if(!g_doublyAdmin)
		var g_doublyAdmin = new UniteAdminDOUBLY();
		...
		switch(type){
			...
			case "mp3":
				...
				if(source != "addon" && jQuery.isNumeric(value) == false)
					value = g_doublyAdmin.urlToRelative(value);
			break;				
			case "post":
	...
	function getInputDefaultValue(objInput){
		...
		switch(type){
			...
			case "checkbox":
				...
				defaultValue = g_doublyAdmin.strToBool(defaultValue);				
			break;
		}
		...
		switch(type){
			...
			default:
				...
				if(objCustomType){
					...
					var success = g_doublyAdmin.clearProviderSetting(type, objInput, dataname);
					if(success == false){
	...
	function setInputValue(objInput, value, value2){
		...
		switch(type){
			...
			break;
			case "checkbox":
				value = g_doublyAdmin.strToBool(value);
			...
			case "radio":
				...
				if(radioValue === "true" || radioValue === "false"){
					radioValue = g_doublyAdmin.strToBool(radioValue);
					value = g_doublyAdmin.strToBool(value);
				}
			...
			default:
				...
				if(objCustomType)
					...
					//check for provider
					var success =  g_doublyAdmin.providerSettingSetValue(type, objInput, value);
				...
				switch(type){
					case "image":
						...
						if(jQuery.isNumeric(value)){
							...
							var url = g_doublyAdmin.getVal(objValues, name+"_url");
							if(url){
								value2 = value;
								value = url;
							}
							...
							var imageID = g_doublyAdmin.getVal(objValues, name+"_imageid");
							if(imageID)
								value2 = imageID;
						}
					break;
	...
	function getCustomSettingType(settingName){
		...
		var objCustomSettings = g_doublyAdmin.getGlobalData(g_temp.customSettingsKey);
		...
		var objType = g_doublyAdmin.getVal(objCustomSettings, settingName, null);
			...
			case "spectrum":
				objInput.spectrum({
					move:function(){
						...
						g_doublyAdmin.runWithTrashold(function(){
	...
	this.updateMp3FieldState = function(objInput, isEnable){
		...
		if(!isEnable){				//set disabled mode
			...
			g_doublyAdmin.disableInput(objInput);
			g_doublyAdmin.disableButton(objButton);
			...
			g_doublyAdmin.enableInput(objInput);
			g_doublyAdmin.enableButton(objButton);
		}
	...
	function onChooseMp3Click(){
		...
		if(g_doublyAdmin.isButtonEnabled(objButton) == false)
			return(true);
		
		var objInput = objButton.siblings(".unite-setting-mp3-input");
		var source = objInput.data("source");
		
		g_doublyAdmin.openAddMp3Dialog(g_uctext.choose_audio,function(urlMp3){
	...
	function setImagePreview(){
		...
		if(source == "addon"){
			...
			urlFull = g_doublyAdmin.urlToFull(url);
			objInput.data("urlfull", urlFull);
			
			url = g_doublyAdmin.urlToFull(urlFull);
			
		}else{
			url = g_doublyAdmin.urlToFull(url);
		}
	...
	function onChooseImageClick(event){
		...
		g_doublyAdmin.openAddImageDialog(g_uctext.choose_image,function(urlImage, imageID){
	...
	this.setAccordionMaxHeight = function(bodyHeight){
		...
		var spaceBetween = g_doublyAdmin.getVal(g_options, "accordion_sap", null);
		if(spaceBetween === null){
			trace(g_options);
			throw new Error("Space between accordion items not set in settings options");
		}
				
		var titleHeight = g_doublyAdmin.getVal(g_options, "accordion_title_height", null);
	...
	this.getObjAddonBrowser = function(addonType){
		...
		var objAddonBrowsersCache = g_doublyAdmin.getGlobalData(keyCache);
		if(!objAddonBrowsersCache)
			objAddonBrowsersCache = {};
		
		var objBrowser = g_doublyAdmin.getVal(objAddonBrowsersCache, addonType);
		...
		if(!objBrowser){
			...
			g_doublyAdmin.validateDomElement(objBrowserWrapper,"addons browser with id: "+browserID);
			
			var objBrowser = new UniteCreatorBrowser();
			objBrowser.init(objBrowserWrapper);
			
			objAddonBrowsersCache[addonType] = objBrowser;
			g_doublyAdmin.storeGlobalData(keyCache, objAddonBrowsersCache);
		}
	...
	function initAddonPicker(objInput){
		...
		objInput.change(function(){
			...
			if(objTitle.length){
				var title = g_doublyAdmin.getVal(objData, "title");
				if(!title)
					title = addonname;
		...
		objButtons.on("click",function(){
			...
			switch(action){
				...
				case "configure":
					...
					g_doublyAdmin.validateNotEmpty(configureAction, "configure action");
					
					g_doublyAdmin.validateDomElement(objInputData, "addon picker input data");
	...
	this.iconPicker_addIconsType = function(name, arrIcons, iconsTemplate, optParams){
		...
		var objType = g_doublyAdmin.getGlobalData(key);
		if(objType)
			return(false);
		...
		g_doublyAdmin.storeGlobalData(key, params);
	};
	...
	function iconPicker_initDialog(type){
		...
		var isAddNew = g_doublyAdmin.getVal(objType, "add_new");
	...
	function initMapPicker(objPickerWrapper){
		...
		g_doublyAdmin.validateDomElement(objButton, "choose map button");
		...
		objButton.on("click",function(){
			...
			g_doublyAdmin.openIframeDialog("mappicker", urlParams, dialogOptions, onMapUpdate);
	...
	function initPostPicker(objWrapper){
		...
		g_doublyAdmin.fixModalDialogSelect2();
		...
		var urlAjax = g_doublyAdmin.getUrlAjax("get_posts_list_forselect");
	...
	function onAnimationSettingChange(){
		...
		g_doublyAdmin.validateDomElement(objParent, "Animation setting parent");
		
		var objDemo = objParent.find(".uc-animation-demo span");
		var animation = objSelect.val();
		
		g_doublyAdmin.validateDomElement(objDemo, "Animation setting demo");
	...
	function onItemsPanelEditItemsClick(){
		...
		g_doublyAdmin.validateDomElement(objDialog, "items dialog");
	...
	this.initFontsPanel = function(objFontsWrapper){
		...
		g_temp.objFontsPanel.find(".uc-fontspanel-toggle").on("click",function(){
			...
			g_doublyAdmin.validateDomElement(objSection, "fonts panel section");
	...
	this.getFontsPanelData = function(){
		...
		jQuery.each(objCheckboxes, function(index, checkbox){
			...
			var objSection = jQuery("#" + sectionID);
			g_doublyAdmin.validateDomElement(objSection, "fonts panel section "+sectionID);
	...
	function setFontPanelData(objInput, arrData){
		...
		jQuery.each(arrData, function(sectionName, objFields){
			...
			g_doublyAdmin.validateDomElement(objSection, "section: "+sectionID);
	...
	function repeaterAddItem(event, objWrapper, objItemValues, objItemInsertAfter){
		...
		g_doublyAdmin.validateDomElement(objItemsWrapper, "items wrapper");
		g_doublyAdmin.validateDomElement(objSettingsTemplate, "settings template");
		...
		g_doublyAdmin.validateDomElement(objItemSettingsWrapper, "item settigns wrapper");
		
		var objOptions = objItemSettingsWrapper.data("options");
		
		//replace setting IDs
		var idPrefix = objOptions.id_prefix;
		var newID = idPrefix+"item_"+g_doublyAdmin.getRandomString(5)+"_";
		html = g_doublyAdmin.replaceAll(html, idPrefix, newID);
		...
		var objItemSettings = new UniteSettingsDOUBLY();
		...
		if(isNewItem == false){
			...
			if(objInputGeneratedID){
				var generatedID = g_doublyAdmin.getRandomString(7);
	...
	function onControlSettingChange(event, input){
		...
		jQuery.each(arrChildControls, function(childName, objControl){
			...
			var arrParents = g_doublyAdmin.getVal(g_arrChildrenControls, childName);
			if(arrParents)
	...
	this.triggerKeyupEvent = function(objInput, event, funcChange){
		...
		g_doublyAdmin.runWithTrashold(funcChange, event, objInput);
		...
		switch(type){
			...
			default:
				...
				if(objCustomType){
					...
					g_doublyAdmin.initProviderSettingEvents(type, objInput);
	...
	function initGlobalEvents(){
		...
		g_doublyAdmin.onEvent("update_assets_path", onUpdateAssetsPath);
	...
	function initOptions(){
		...
		jQuery.each(arrOptions, function(index, optionKey){
			g_options[optionKey] = g_doublyAdmin.getVal(objOptions, optionKey, g_options[optionKey]);
	...
	this.destroy = function(){
		...
		g_doublyAdmin.offEvent("update_assets_path");
	...
	this.addCustomSettingType = function(type, objType){
		...
		g_doublyAdmin.validateObjProperty(objType, ["funcInit",
                               "funcSetValue",
                               "funcGetValue",
                               "funcClearValue",
                               "funcDestroy",
		...
		var existing = g_doublyAdmin.getVal(objCustomSettings, type);
		if(existing)
			throw new Error("The custom settings type: "+type+" alrady exists");
		
		objCustomSettings[type] = objType;
		
		g_doublyAdmin.storeGlobalData(g_temp.customSettingsKey, objCustomSettings);
	};

 ...
 */
	class UniteSettingsDOUBLY{
	    ...
	    public function __construct(){
	    	...
	    	$suffix = time()."_".UniteFunctionsDOUBLY::getRandomString(5)."_";
		...
		private function modifySettingsData($arrSettings){
			...
			foreach($arrSettings as $key=>$content){
				switch(getType($content)){
					...
					case "object":
					case "array":
						$content = UniteFunctionsDOUBLY::convertStdClassToArray($content);
					break;					
				}
		...
		public function getArrSettingNamesAndTitles(){
			...
			foreach($this->arrSettings as $setting){
				$name = UniteFunctionsDOUBLY::getVal($setting, "name");
				$title = UniteFunctionsDOUBLY::getVal($setting, "text");
				if(!empty($name))
					$arrNames[$name] = $title;
			}
		...
		public function getSap($sapKey){
			...
			$arrSap = UniteFunctionsDOUBLY::getVal($this->arrSaps, $sapKey);
			if(empty($arrSap))
				UniteFunctionsDOUBLY::throwError("sap with key: $sapKey not found");
		...
		private function getIndexByName($name){
			...
			if(!empty($this->arrIndex)){
				if(array_key_exists($name, $this->arrIndex) == false)
					UniteFunctionsDOUBLY::throwError("setting $name not found");
			...
			foreach($this->arrSettings as $index=>$setting){
				$settingName = UniteFunctionsDOUBLY::getVal($setting, "name");
				if($settingName == $name)
					return($index);
			}
			
			UniteFunctionsDOUBLY::throwError("Setting with name: $name don't exists");
		}
		...
		public function getArrSettingsBySap($sapName){
			...
			if($sapKey === null)
				UniteFunctionsDOUBLY::throwError("sap: $sapName not found");
		...
		public function getArrTextFromAllSettings(){
			...
			foreach($this->arrSettings as $setting){
				...
				$desc = UniteFunctionsDOUBLY::getVal($setting, "description");
				...
				$items = UniteFunctionsDOUBLY::getVal($setting, "items");
				if(!empty($items)){
		...
		public function getSettingValue($name,$default=""){
			...
			$value = UniteFunctionsDOUBLY::getVal($setting, "value",$default);
		...
		public function addHr($name="",$params=array()){
			...
			if($name != "")
			  ...
			  $this->HRIdCounter++;
			  $itemName = "hr_".UniteFunctionsDOUBLY::getRandomString();
		...
		public function addSap($text, $name="", $opened = false, $icon="", $params = array()){
			...
			if(empty($text))
				UniteFunctionsDOUBLY::throwError("sap $name must have a text");
		...
		protected function addSettingGlobalParams($setting){
			...
			$settingType = UniteFunctionsDOUBLY::getVal($setting, "type");
			foreach($this->arrGlobalParams as $param){
				
				$types = UniteFunctionsDOUBLY::getVal($param, "types");
		...
		protected function add($name,$defaultValue = "",$text = "",$type = self::TYPE_TEXT,$arrParams = array()){
			...
			$modifyType = UniteFunctionsDOUBLY::getVal($setting, "modifytype");
			if(!empty($modifyType)){				
		...
		public function addSettingByArray($setting){
			...
			if(is_array($setting) == false)
				UniteFunctionsDOUBLY::throwError("addSettingByArray: Empty setting given");
			
			$name = UniteFunctionsDOUBLY::getVal($setting, "name");
			
			UniteFunctionsDOUBLY::validateNotEmpty($name, "setting name");
		...
		public function addControl($control_item_name,$controlled_item_name,$control_type,$value){
			...
			UniteFunctionsDOUBLY::validateNotEmpty($control_item_name,"control parent");
			UniteFunctionsDOUBLY::validateNotEmpty($controlled_item_name,"control child");
			UniteFunctionsDOUBLY::validateNotEmpty($control_type,"control type");
		...
		public function setPairedSettings(){
			...
			foreach($this->arrSettings as $setting){
				$addSettingName = UniteFunctionsDOUBLY::getVal($setting, self::PARAM_ADDFIELD);
		...
		public function loadXMLFile($filepath, $loadedSettingsType = null){
			...
			if(!file_exists($filepath))
				UniteFunctionsDOUBLY::throwError("File: '$filepath' not exists!!!");
			
			if(function_exists("simplexml_load_file") == false)
				UniteFunctionsDOUBLY::throwError("Your php missing SimpleXML Extension. The plugin can't work without this extension because it has many xml files to load. Please enable this extension in php.ini");
			
			$obj = simplexml_load_file($filepath);
			
			if(empty($obj))
				UniteFunctionsDOUBLY::throwError("Wrong xml file format: $filepath");
			...
			foreach($fieldsets as $fieldset){
				...
				$sapName = (string)UniteFunctionsDOUBLY::getVal($attribs, "name");
				$sapLabel = (string)UniteFunctionsDOUBLY::getVal($attribs, "label");
				$sapIcon = (string)UniteFunctionsDOUBLY::getVal($attribs, "icon");				
				$loadFrom = (string)UniteFunctionsDOUBLY::getVal($attribs, "loadfrom");	
				$loadParam = (string)UniteFunctionsDOUBLY::getVal($attribs, "loadparam");	
				$loadedSettingsType = (string)UniteFunctionsDOUBLY::getVal($attribs, "loadtype");	
				$nodraw = (string)UniteFunctionsDOUBLY::getVal($attribs, "nodraw");				
				$visibility = (string)UniteFunctionsDOUBLY::getVal($attribs, "visibility");				
				...
				*/
				...
				$sapParams = array();
				if(!empty($nodraw)){
					$sapParams["nodraw"] = UniteFunctionsDOUBLY::strToBool($nodraw);
				}
				
				UniteFunctionsDOUBLY::validateNotEmpty($sapName,"sapName");
				...
				UniteFunctionsDOUBLY::validateNotEmpty($sapLabel,"sapLabel");
				...
				$fields = UniteFunctionsDOUBLY::getVal($fieldset, "field");
				...
				foreach($fields as $field){
					...
					$fieldType = (string)UniteFunctionsDOUBLY::getVal($attribs, "type");
					$fieldName = (string)UniteFunctionsDOUBLY::getVal($attribs, "name");
					$fieldLabel = (string)UniteFunctionsDOUBLY::getVal($attribs, "label");
					$fieldDefaultValue = (string)UniteFunctionsDOUBLY::getVal($attribs, "default");
					...
					UniteFunctionsDOUBLY::validateNotEmpty($fieldType,"type");
					
					//validate name
					if($fieldType != self::TYPE_HR && $fieldType != self::TYPE_CONTROL &&
						$fieldType != "bulk_control_start" && $fieldType != "bulk_control_end")
						UniteFunctionsDOUBLY::validateNotEmpty($fieldName,"name");		
					switch ($fieldType){
						case self::TYPE_CHECKBOX:
							
							$fieldDefaultValue = UniteFunctionsDOUBLY::strToBool($fieldDefaultValue);
						...
						case self::TYPE_CONTROL:
							$parent = UniteFunctionsDOUBLY::getVal($arrParams, "parent");
							$child =  UniteFunctionsDOUBLY::getVal($arrParams, "child");
							$ctype =  UniteFunctionsDOUBLY::getVal($arrParams, "ctype");
							$value =  UniteFunctionsDOUBLY::getVal($arrParams, "value");
						...
						break;	
						default:
							UniteFunctionsDOUBLY::throwError("wrong type: $fieldType");
						break;						
					}
		...
		public function addFromXmlFile($filepathXML){
			...
			$objNewSettings = new UniteSettingsDOUBLY();
		...
		private function getOptionsFromXMLField($field, $fieldName){
			...
			$options = UniteFunctionsDOUBLY::getVal($arrField, "option");
			...
			foreach($options as $option){
				...
				if(gettype($option) == "string")
					UniteFunctionsDOUBLY::throwError("Wrong options type: ".$option." in field: $fieldName");
				
				$attribs = $option->attributes();
				
				$optionValue = (string)UniteFunctionsDOUBLY::getVal($attribs, "value");							
				$optionText = (string)UniteFunctionsDOUBLY::getVal($attribs, "text");
				
				//validate options:
				UniteFunctionsDOUBLY::validateNotEmpty($optionText,"option text");
		...
		public function mergeSettings($settings){
			...
			foreach($arrSettingsNew as $setting){
				...
				if(array_key_exists($sapOld, $arrNewSapKeys) == false)
					UniteFunctionsDOUBLY::throwError("sap {$sapOld} should be exists in sap keys array");
				...
				if(array_key_exists($name, $this->arrIndex))
					UniteFunctionsDOUBLY::throwError("The setting <b>{$name} </b> already exists. ");
		...
		private function addExternalSettings($filename, $loadParam = null, $loadType = null){
			...
			$filepathSettings = GlobalsDOUBLY::$pathSettings."{$filename}.xml";
			
			if(file_exists($filepathSettings) == false)
				UniteFunctionsDOUBLY::throwError("The file: {$filename}.xml don't found in settings folder");
		...
		public function updateArrSettingByName($name,$setting){
			...
			foreach($this->arrSettings as $key => $settingExisting){
				$settingName = UniteFunctionsDOUBLY::getVal($settingExisting,"name");
				if($settingName == $name){
		...
		public function modifyValueByDatatypeFunc($value, $datatype){
			...
			switch($datatype){
				...
				case self::DATATYPE_LINK:
					...
					$link = UniteFunctionsDOUBLY::getHrefFromHtml($value);
					if(!empty($link))
						$value = $link;
		...
		public function setStoredValues($arrValues){
			...
			foreach($this->arrSettings as $key=>$setting){
				...
				$datatype = UniteFunctionsDOUBLY::getVal($setting, "datatype");
								
				//skip custom type
				$customType = UniteFunctionsDOUBLY::getVal($setting, "custom_type");

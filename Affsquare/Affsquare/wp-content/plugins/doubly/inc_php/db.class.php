...
if(!defined("DOUBLY_INC")) die("restricted access");
	...
	class Doubly_PluginDB{
		...
		public function __construct(){
			...
			$this->pdb = new Doubly_PluginProviderDB();
		...
		private function throwError($message,$code=-1){
			UniteFunctionsDOUBLY::throwError($message,$code);
		}
		...
		public function delete($table,$where){
			...
			UniteFunctionsDOUBLY::validateNotEmpty($table,"table name");
			UniteFunctionsDOUBLY::validateNotEmpty($where,"where");
		...
		public function getWhereCatIDWithAll($catID){
			...
			if($catID === "all"){
				...
				if(is_array($catID) == false)
					UniteFunctionsDOUBLY::throwError("catIDs could be array or number");
		...
		public function update($tableName,$arrData,$where){
			...
			UniteFunctionsDOUBLY::validateNotEmpty($tableName,"table cannot be empty");
			UniteFunctionsDOUBLY::validateNotEmpty($where,"where cannot be empty");
			UniteFunctionsDOUBLY::validateNotEmpty($arrData,"data cannot be empty");
		...
		public function fetchSql($query, $supressErrors = false){
			...
			$rows = UniteFunctionsDOUBLY::convertStdClassToArray($rows);
		...
		public function updateRecordsOrdering($table, $arrIDs){
			...
			//get items assoc
			$arrRecords = UniteFunctionsDOUBLY::arrayToAssoc($arrRecords,"id");
			
			$order = 0;
			foreach($arrIDs as $recordID){
				$order++;
				
				$arrRecord = UniteFunctionsDOUBLY::getVal($arrRecords, $recordID);
		...
		public function fetchPage($tableName, $pagingOptions, $where="", $orderField="", $groupByField="", $sqlAddon=""){
			...
			$page = UniteFunctionsDOUBLY::getVal($pagingOptions, "page");
			$rowsInPage = UniteFunctionsDOUBLY::getVal($pagingOptions, "inpage");
			
			
			//valdiate and sanitize
			UniteFunctionsDOUBLY::validateNumeric($page);
			UniteFunctionsDOUBLY::validateNumeric($rowsInPage);
			UniteFunctionsDOUBLY::validateNotEmpty($rowsInPage);
			if($page < 1)
				$page = 1;
		...
		public function fetchSingle($tableName,$where="",$orderField="",$groupByField="",$sqlAddon=""){
			...
			if(is_array($tableName)){
				...
				$tableName = UniteFunctionsDOUBLY::getVal($arguments, "tableName");
				$where = UniteFunctionsDOUBLY::getVal($arguments, "where");
				$orderField = UniteFunctionsDOUBLY::getVal($arguments, "orderField");
				$groupByField = UniteFunctionsDOUBLY::getVal($arguments, "groupByField");
				$sqlAddon = UniteFunctionsDOUBLY::getVal($arguments, "sqlAddon");
				$errorEmpty = UniteFunctionsDOUBLY::getVal($arguments, "errorEmpty");
			}
			
			if(empty($errorEmpty)){
				$tableTitle = UniteFunctionsDOUBLY::getVal(self::$arrTableTitles, $tableName, __("Record", "unlimited-elements-for-elementor"));
